...
def concat_data(file_lists, i):
    ...
    for file in human_pose_list:
        temp_csv = pd.read_csv(os.path.join('/home/graham/Code/Kimore',file), header=None)
    ...
    for robot_file in robot_joint_list:
        temp_robot_csv = pd.read_csv(os.path.join('/home/graham/Code/Kimore',robot_file))
...
class KimoreLoader(Dataset):
    def __init__(self, index , flag='train', data_root='/home/graham/Code/Kimore',scale=True):
        # size [seq_len, label_len, pred_len]
        # info
        self.seq_len = 16
        self.label_len = 4
    ...
    def __read_data__(self):
        self.scaler = StandardScaler()
        human_xyz, human_quat, robot_xyz, robot_quat, robot_force = concat_data(self.file_list, self.index)

        self.data_x = human_xyz
        self.data_y = robot_xyz
